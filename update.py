...
with open('k:/Antigravity Projects/Marketplace/hero.html', 'r', encoding='utf-8') as f:
    content = f.read()

# Replace CSS
css_old = r'        /\* ========== WHY CUSTOMERS LOVE — BENTO GRID ==========\*/.*?            \.why-inner \{\n                padding: 0 24px;\n            \}\n        \}'
css_old2 = r'        /\* ========== WHY CUSTOMERS LOVE — BENTO GRID ==========\s*\*/.*?@media \(max-width: 768px\) \{.*?padding: 0 24px;\s*\}\s*\}'

css_new = '''        /* ========== WHY PHIMART — SCROLL INTERACTIVE ========== */
        .why-premium {
            position: relative;
            z-index: 1;
            width: 100%;
...
content = re.sub(css_old2, css_new, content, flags=re.DOTALL)

# Replace HTML
html_old = r'    <!-- WHY CUSTOMERS LOVE PHIMART -->\s*<section class="why-love">.*?</section>'
html_new = '''    <!-- WHY PHIMART — SCROLL INTERACTIVE SECTION -->
